...
impl UprobeHandler {
    pub fn attach_app(pid: i32) -> Result<Self> {
        ...
        handler.symbol = if handler.attach_symbol(pid, SYMBOL_SINGLE).is_ok() {
            SYMBOL_SINGLE
        } else if handler.attach_symbol(pid, SYMBOL_BATCH).is_ok() {
            SYMBOL_BATCH
        } else {
            handler.attach_symbol(pid, SYMBOL_FALLBACK)?;
            SYMBOL_FALLBACK
        };
        handler.attached_tids.insert(pid);
    ...
    fn refresh_threads_inner(&mut self, pid: i32) -> Result<()> {
        ...
        for tid in tids {
            if self.attach_symbol(tid, symbol).is_ok() {
    ...
    fn attach_symbol(&mut self, tid: i32, symbol: &str) -> Result<()> {
        self.get_program()?
            .attach(Some(symbol), 0, LIBGUI_PATH, Some(tid))?;
        Ok(())
    }
